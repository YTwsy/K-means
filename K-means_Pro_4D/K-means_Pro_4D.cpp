...
{
    return sqrt((a.x - b.x) * (a.x - b.x) + (b.y - a.y) * (b.y - a.y)+pow((a.area-b.area),2)+pow((a.leng - b.leng),2));
}
...
{
    ifstream ifs;
    ifs.open("my_pre_cluster_points.txt", ios::in);
    if (!ifs.is_open())
    {
...
void KMeans::first_k()
{
    for (int i = 0; i < K; i++)
    {
        ...
        k_center[i].area = points[first_center_id].area;
        k_center[i].leng = points[first_center_id].leng;
...
void KMeans::first_k_pro()
{
    ...
    k_center[0].area = points[first_center_id].area;
    k_center[0].leng = points[first_center_id].leng;
    ...
    for (int j = 0; j < K; j++)
    {
        for (int i = 0; i < point_count; i++)
        {
            ...
            k_center[j].area = points[dis[0].id].area;
            k_center[j].leng = points[dis[0].id].leng;
...
bool KMeans::change_center()
{
    ...
    for (int k_class_i = 0; k_class_i < K; k_class_i++)
    {
        ...
        double new_center_l = 0;
        double new_center_a = 0;
        ...
        for (int i = 0; i < every_k_class_with_point[k_class_i].size(); i++)
        {
            ...
            new_center_a += every_k_class_with_point[k_class_i][i].area;
            new_center_l += every_k_class_with_point[k_class_i][i].leng;
        ...
        new_center_a = new_center_a / every_k_class_with_point[k_class_i].size();
        new_center_l = new_center_l / every_k_class_with_point[k_class_i].size();


        double center = (new_center_x - k_center[k_class_i].x) *
            (new_center_x - k_center[k_class_i].x) + (new_center_y - k_center[k_class_i].y) *
            (new_center_y - k_center[k_class_i].y)+ (new_center_a - k_center[k_class_i].area) *
            (new_center_a - k_center[k_class_i].area) + (new_center_l - k_center[k_class_i].leng) *
            (new_center_l - k_center[k_class_i].leng);        //接口准备

        center_from_pre[k_class_i] = center;

        k_center[k_class_i].x = new_center_x;
        k_center[k_class_i].y = new_center_y;
        k_center[k_class_i].area = new_center_a;
        k_center[k_class_i].leng= new_center_l;
...
void KMeans::write_to_txt()
{
    ...
    string K_to_name = "k - means_point.txt";
    //stringstream ss;
    //string str;
    //ss << K;
    //ss >> str;
